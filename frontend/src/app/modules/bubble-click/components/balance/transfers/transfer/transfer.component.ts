  ...
  template: `
    <section class="accent-border accent-border-top accent-bg-shadow rounded-5 tg-bg-secondary">
      <div class="hstack p-3 pb-0 color-accent">
        <span class="m-auto text-center h5">{{ localisation.messages.Transfer ?? 'Transfer' }} ({{ this.localisation.messages.max ?? 'max.' }}: {{ this.coinsService.balanceSubject | async }})</span>
      </div>
...
export class TransferComponent extends ReactiveForm implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.twa.setMainButton({text: this.localisation.messages.Transfer ?? 'Transfer', is_active: true, is_visible: true}, this.transfer)
  }
